...
class TestNoteListView:
    ...
    def test_note_list_filter_by_type(self, client):
        ...
        url = reverse("notes:note_list") + "?type=Bible+Study"
        response = client.get(url)
        ...
        url = reverse("notes:note_list") + "?type=Sermon+Notes"
        response = client.get(url)
    ...
    def test_note_list_filter_by_book(self, client):
        ...
        # Filter by Genesis
        url = reverse("notes:note_list") + f"?book={genesis.id}"
        response = client.get(url)
    ...
    def test_note_list_filter_by_tag(self, client):
        ...
        # Filter by faith tag
        url = reverse("notes:note_list") + "?tag=faith"
        response = client.get(url)
    ...
    def test_note_list_search(self, client):
        ...
        url = reverse("notes:note_list") + "?q=salvation"
        response = client.get(url)
...
class TestNoteDetailView:
    def test_login_required(self, client):
        ...
        note = NoteFactory()
        url = reverse("notes:note_detail", kwargs={"pk": note.pk})
        response = client.get(url)
        assert response.status_code == 302  # Redirect to login page
...
class TestNoteEditPageView:
    def test_login_required(self, client):
        ...
        note = NoteFactory()
        url = reverse("notes:note_edit", kwargs={"pk": note.pk})
        response = client.get(url)
        assert response.status_code == 302  # Redirect to login page
...
class TestNoteDeleteView:
    def test_login_required(self, client):
        ...
        note = NoteFactory()
        url = reverse("notes:note_delete", kwargs={"pk": note.pk})
    ...
    def test_delete_own_note(self, client):
        ...
        assert response.url == reverse("notes:note_list")
