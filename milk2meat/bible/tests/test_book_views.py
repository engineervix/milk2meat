...
class TestBookDetailView:
    def test_login_required(self, client):
        ...
        book = BookFactory()
        url = reverse("bible:book_detail", kwargs={"pk": book.pk})
        response = client.get(url)
        assert response.status_code == 302  # Redirect to login page
...
class TestBookEditPageView:
    def test_login_required(self, client):
        ...
        book = BookFactory()
        url = reverse("bible:book_edit", kwargs={"pk": book.pk})
        response = client.get(url)
        assert response.status_code == 302  # Redirect to login page
    ...
    def test_book_edit_page_view_get(self, client):
        ...
        assert response.context["book_update_url"] == reverse("bible:book_update_ajax", kwargs={"pk": book.pk})
